  ...
  const SmallBook: React.FC<ISmallBook> = ({ image,title,author,rating }) => {
    const returnRating=(rating: any)=>{
      ...
      for(let i=0; i< rating; i++){
       stars.push(<span key={i}><AiFillStar size={15}/></span>)
      }
      return stars
    }
    return(
      <div>
              <Image className="rounded-sm" src={image} alt="cover image for books" width={100} height={100}/>
        <div className="flex flex-col list-none justify-center">
            <li className="font-bold">{title}</li>
            <li className="text-sm font-semibold ">{author}</li>
